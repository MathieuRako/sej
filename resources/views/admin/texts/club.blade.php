...
@section('content')
    ...
    {!! texts_update_paragraph($alltexts['title-value'][1], $alltexts['paragraph-value'][1], null, 2, 'club', true) !!}
    {!! texts_update_paragraph($alltexts['title-value'][2], $alltexts['paragraph-value'][2], null, 3, 'club', true) !!}
    {!! texts_update_paragraph($alltexts['title-history'][0], [$alltexts['paragraph-history'][0],$alltexts['paragraph-history'][1]], null, 4, 'club', true) !!}
    {!! texts_update_form(
        [
                ...
                <div class="card-body">
                    ...
                    <form action="/admin/texts" method="POST" id="form-4">
                        ...
                        @foreach($alltexts['rule'] as $r)
                            {!! texts_field(['label' => 'Rule', 'row' => 1, 'col' => 50, 'text' => $r], 4, null) !!} <a href = "/admin/texts/remove?id={{ $r['id'] }}&page=club"><i class="fa-solid fa-circle-minus"></i></a><br>
                            <?php
                            $ids[] = $r['id'];
                            ?>
                        @endforeach
                        {!! texts_update_submit($ids, 'club') !!}
                    </form>
       
                    {!! texts_add_form( [
                        ['label' => 'Add rule', 'name'=> 'content', 'row' => 1, 'col' => 50, 'text' => ' ']], 5, 'club', [['name' => 'type', 'value' => 'rule']], false)!!}
                </div>
            </div>
        </div>
    </div>
